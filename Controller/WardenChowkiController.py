...
class WardenChowkiController:
    ...
    @staticmethod
    def generate_warden_number(cityname):
        # Get the last Warden's ID
        last_warden = db.session.query(TrafficWarden).order_by(TrafficWarden.id.desc()).first()
        next_id = last_warden.id + 1 if last_warden else 1

        return f"W{cityname[0]}-{next_id:04d}"  # Format as W-0001, W-0002, etc.

    @staticmethod
    def add_warden(name, address, cnic, email, mobile_number, city_name):
        ...
        temp_password = WardenChowkiController.generate_temp_password()
        hashed_password = generate_password_hash(temp_password)
        badge_number=WardenChowkiController.generate_warden_number(city_name)
        print("Temporary Password:", temp_password)
        print("Hashed Password for DB:", hashed_password)
    ...
    #
    #         active_shift = next((shift for shift in shifts if shift.start_time <= current_time < shift.end_time), None)
    #
    #         if active_shift:
    #             db.session.query(TrafficWarden).update({TrafficWarden.status: 'OFF_DUTY'})
    #             active_wardens = db.session.query(WardenChowki).filter(
    #                 WardenChowki.shift_id == active_shift.id).all()
    #
    #             for assignment in active_wardens:
    #                 db.session.query(TrafficWarden).filter(TrafficWarden.id == assignment.warden_id).update(
    #                     {TrafficWarden.status: 'ON_DUTY'}
    #                 )
    #
    #             db.session.commit()
    #             print(f"Warden status updated for shift: {active_shift.name}")
    #         else:
    #             print("No active shift found at this time.")
    #
    #     except Exception as e:
    #         db.session.rollback()
    #         print(f"Error updating warden status: {e}")
    #
    #     finally:
    #         db.session.close()
    #
    # # Run the function continuously every 60 seconds
    # while True:
    #     update_warden_status()
